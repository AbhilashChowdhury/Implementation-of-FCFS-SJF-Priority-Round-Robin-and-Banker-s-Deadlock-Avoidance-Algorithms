def sjf(processes, burst_times, arrival_times):
    ...
    remaining_processes = sorted(zip(processes, burst_times, arrival_times), key=lambda x: x[2])
    
    current_time, completed = 0, 0
    while completed < n:
        ready_queue = [p for p in remaining_processes if p[2] <= current_time]
        if ready_queue:
            process, burst_time, arrival_time = min(ready_queue, key=lambda x: x[1])
            remaining_processes.remove((process, burst_time, arrival_time))
            ...
            current_time = remaining_processes[0][2]
    
    print("\nProcess\tAT\tBT\tCT\tTAT\tWT")
    for i in range(n):
        print(f"P{processes[i]}\t{arrival_times[i]}\t{burst_times[i]}\t{completion_times[i]}\t{turnaround_times[i]}\t{waiting_times[i]}")
